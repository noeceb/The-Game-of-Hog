...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    def check_nums(score):
        num1 = score % 10
        score = score // 10
        num2 = score % 10
        absolute_val = abs(num1 - num2)
        return absolute_val

    return check_nums(player_score) == check_nums(opponent_score)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def aggregate(score0, score1):
        current_high = previous_high
        if who == 0:
            difference = score0 - previous_score
            if difference > current_high:
                current_high = difference
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    num, high_average, best_roll = 1, 0, 0
    while num <= 10:
        i = average_roll(num, dice)
        if i > high_average:
            high_average = i
            best_roll = num
        num +=  1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap((free_bacon(opponent_score) + score), opponent_score):
        if opponent_score > (free_bacon(opponent_score) + score):
            return 0
    if free_bacon(opponent_score) >= margin:
        if is_swap((free_bacon(opponent_score) + score), opponent_score) is False:   
            return 0                                    
    return num_rolls  
    # END PROBLEM 11
